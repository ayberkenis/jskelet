...
export default {
  ...
  home: {
    ...
    ops: {
      ...
      title: "Redis, an admin panel, Cloudflare — without bolting on a second product.",
      lead: "The HTML cache starts in process memory. Flip a switch and the same keys share across replicas, open a hardened panel, and talk to your edge.",
      tabsLabel: "Choose a story",
      stories: [
        {
          id: "redis",
          tab: "Redis L2",
          kicker: "Shared tier",
          title: "Memory first. Redis when you scale out.",
          body: "L1 stays in process for microsecond hits. Redis holds the shared HTML/data bodies and fans invalidateHtmlCache() / clearHtmlCache() to every replica over pub/sub — ioredis is optional; missing it just means L1-only.",
          facts: [
            { label: "Primary store", value: "In-process L1" },
            { label: "Shared bodies", value: "Redis HTML + data" },
            { label: "Fan-out", value: "Pub/sub invalidate" },
          ],
        },
        {
          id: "admin",
          ...
          title: "A real panel for cache, routes, views and live logs.",
          body: "Overview, Cache, Routes, Views, Logs and System. Password lands in the server log each boot, strangers get 404, bots are blocked by default, and every action needs the panel CSRF header.",
          facts: [
            { label: "Enable", value: "admin() or JSKELET_ADMIN" },
            { label: "Live logs", value: "In-process SSE" },
            { label: "Bodies", value: "Never exported" },
          ],
          nav: ["Overview", "Cache", "Routes", "Views", "Logs", "System"],
          cards: [
            { label: "HTML", value: "128", note: "entries · HIT 94%" },
            { label: "Redis", value: "on", note: "shared · pub/sub" },
            { label: "Prewarm", value: "400", note: "paths / pass" },
            { label: "Upstream", value: "12/s", note: "limiter" },
          ],
          logLines: [
            { method: "GET", path: "/news/markets", cache: "HIT", ms: "2" },
            { method: "GET", path: "/news/rates", cache: "STALE", ms: "4" },
            { method: "PURGE", path: "cf · /news/*", cache: "OK", ms: "86" },
          ],
        },
        {
          id: "cloudflare",
          tab: "Cloudflare",
          kicker: "Edge link",
          title: "Purge the edge from the same place you purge L1.",
          body: "cache().cloudflare wires zone credentials so the admin Cache page can purge everything, listed URLs, a prefix, a host or a tag — and surface zone settings when you need them during an incident.",
          facts: [
            { label: "Purge", value: "URL · prefix · tag · host" },
            { label: "Analytics", value: "Pinned 24h window" },
            { label: "Same panel", value: "Cache → Cloudflare" },
          ],
          steps: [
            { label: "App", detail: "invalidateHtmlCache" },
            { label: "Redis", detail: "fan-out replicas" },
            { label: "Cloudflare", detail: "purgeCloudflare()" },
            { label: "Visitor", detail: "fresh HTML" },
          ],
        },
      ],
    },
