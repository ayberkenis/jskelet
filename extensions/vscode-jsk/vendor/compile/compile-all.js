 ...
 */
export async function compileAll(config, _options = {}) {
  ...
  const manifest = {};
  /** @type {Map<string, { code: string, includes: string[], includeEntries: { id: string, index: number }[], source: string, relative: string }>} */
  ...
  for (const [viewId, file] of jskFiles) {
    ...
    const { code, includes, includeEntries } = codegen(ast, {
      viewId,
      file: relative,
      knownComponents,
    });

    compiled.set(viewId, { code, includes, includeEntries, source, relative });
  }

  // Include hedefleri mevcut mu?
  for (const [, { includeEntries, source, relative }] of compiled) {
    for (const { id, index } of includeEntries) {
      const normalized = normalizeIncludeId(id);
      if (!compiled.has(normalized) && !jskFiles.has(normalized)) {
        throw new CompileError(`Unknown include "${id}"`, {
          file: relative,
          source,
          index,
        });
