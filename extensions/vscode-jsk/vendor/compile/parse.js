...
class TemplateParser {
  ...
  constructor(source, file) {
    ...
    this.file = file;
    this.i = 0;
    /** @type {string[]} Açık {#if}/{#each} kapanışları — EOF'ta öncelikli hata. */
    this.blockStack = [];
  }
  ...
  parseNodes(until) {
    ...
    while (!this.eof()) {
      ...
      // `<!DOCTYPE …>` / `<![CDATA[…]]>` — etiket değil; metin olarak bas.
      // İlerletmezsek `<` + `!` dalı parseText'te de takılıp sonsuz döngü olur.
      if (this.startsWith("<!")) {
  ...
  /**
   * `<!DOCTYPE html>` gibi bildirimler. Kapanış `>` ilk tırnak dışı geçiş.
   * @returns {TextNode}
   */
  parseDeclaration() {
    const start = this.i;
  ...
  parseIf(index) {
    ...
    let alternate = [];
    try {
      consequent = this.parseNodesUntil(["{#else}", "{/if}"]);
  ...
  parseEach(index) {
    ...
    this.i = closeBrace + 1;
    this.blockStack.push("{/each}");
    /** @type {AstNode[]} */
    let children;
    try {
      children = this.parseNodesUntil(["{/each}"]);
    } finally {
      this.blockStack.pop();
    }
    if (!this.startsWith("{/each}")) {
  ...
  parseTag() {
    ...
    if (!selfClosing && !voidEl) {
      ...
      if (!this.startsWith(`</${name}>`)) {
        if (this.eof() && this.blockStack.length) {
          throw this.error(`Expected ${this.blockStack.at(-1)}`, this.i);
        }
